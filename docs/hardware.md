...
| Ethernet switch | MT7530 gigabit switch, 1x WAN + 2x LAN | confirmed upstream (DTS switch port map) |
| Wi-Fi 2.4 GHz | MediaTek MT7603EN (802.11n, 2 spatial streams — chip capability); EEPROM signature word `0x7603` at Factory offset 0x0000 | confirmed from dump (Factory signatures) + upstream (`kmod-mt7603` in the device profile) |
| Wi-Fi 5 GHz | MediaTek MT7615N (802.11ac, 4 spatial streams — chip capability); EEPROM signature word `0x7615` at Factory offset 0x8000 | confirmed from dump (Factory signatures) + upstream (`kmod-mt7615-firmware` in the device profile) |
